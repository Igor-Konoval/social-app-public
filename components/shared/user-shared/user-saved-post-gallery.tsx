...
export const UserSavedPostGallery: React.FC<{
	posts: TypeSavedPosts
}> = ({ posts }) => {
	return (
		<>
			{posts && posts.length ? (
						...
						>
							{post.post.imageUrl[0] ? (
								<Image
									...
									alt={`Post ${post.id}`}
									className='w-auto max-h-[140px] [@media(min-width:390px)]:max-h-[172px] rounded-lg'
								/>
							) : (
							...
							<div className='absolute flex flex-col items-center inset-0 bg-black bg-opacity-50 justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg'>
								<span className='mr-3 flex flex-row items-center gap-1 text-white text-sm'>
									<Heart className='w-4 h-4' /> {post.post._count.reactions}
								</span>
